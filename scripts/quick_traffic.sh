#!/bin/bash
# Script rapide pour générer du trafic API
...
for ((i=1; i<=COUNT; i++)); do
    # Alterner entre baseline et secured
    if [ $((i % 3)) -eq 0 ]; then
        MODEL="secured"
    else
        MODEL="baseline"
    ...
    echo "[$i/$COUNT] Testing $MODEL with $CAT image..."
    curl -s -X POST "$API_URL/predict/$MODEL" -F "file=@$IMAGE" | python -c "import sys, json; d=json.load(sys.stdin); print(f\"  => Prediction: {d.get('prediction')}, Confidence: {d.get('confidence', 0):.3f}\")" 2>/dev/null || echo "  => Error"
